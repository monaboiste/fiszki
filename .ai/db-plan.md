...
- `flashcards` (1) -- (N) `generations`

  - Kolumna `flashcard_id` w tabeli generations odnosi się do `flashcards.flashcard_id`.
...
- Indeks na kolumnie `generations.flashcard_id` dla szybkiego dostępu do generacji powiązanych z daną fiszką.
...
- Wszystkie kolumny dat (`created_at`, `updated_at`, `generated_at`, `logged_at`) są zapisywane z użyciem TIMESTAMPTZ, aby zachować zgodność z czasem UTC.
